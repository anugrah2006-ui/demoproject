...
export async function POST(req: NextRequest) {
  try {
    ...
    const {
      data: { user },
    } = await getUserWithTrace(supabase, 'api-search-route', {
      pathname: '/api/search',
    });
    ...
    // Force real-time search context to hint router if needed, though router is smart enough.
    ...
    const customStream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          logger.error('Stream processing error in /search', { error: streamError });
    ...
    logger.error('API /search error', { error: errorMessage });
